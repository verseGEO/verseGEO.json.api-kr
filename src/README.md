...
|-----|---|---|--------|---------||
|merchantInformation.merchantId|Y|50|Channel number|Channel number allocated by MW30P|Respond the same as the requested value|
|merchantInformation.merchantSiteId|Y|30|Channel sub-number|Subchannel number assigned by MW30P|Respond the same as the requested value|
|clientReferenceInformation.code|Y|20|Transaction number|Transaction generated by channel only (ex) System ID or Server ID+yyyMMdd+hhmmss+miliscond)|Respond the same as the requested value|
|fromCurrency|Y|10|Request unit|Conversion request currency code (Item, Token, etc.)|GOLD|
|toCurrency|Y|10|Conversion unit|Conversion target currency code (Item, Token, etc.)|SLAYB|
|fromAmount|Y|20|Request Quantity|Item, Point, Token quantity (Error returned when requested below the minimum unit specified by the exchange rate registration administrator)|100|
|toAmount|N|20|Conversion Quantity|Conversion quantity = Requested quantity * Exchange rate|1|
|exchangeRate|N|20|Exchange Rate|Use Exchange rate received by Exchange Rate Inquiry API|10%|
|status|Y|10|Result|Success (SUCCSS), Failure (DECLINED)|SUCCESS, DECLINED|
|errorInformation.errCd|N|8|Error code|NULL for Success, Code for Error|See Error Code|
|errorInformation.reason|N|192|Error message|Corresponding error message when an error occurs|See Error Code|
|sign|Y|64|Signature verification value|Security Signatures (see "2. Security Enforcement Guide")|DEDC93DB5CFE0F06CBB54B937266D378C27E2DE985E999B7F319666857E6C9EE|
